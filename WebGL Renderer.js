function defineWebGLRenderer(bound, debug = false) {
	let gl = null;
	const glState = {};
	const glAttributes = {};
	const glShaders = {};
	...
	function updateResolution() {
		...
		gl.uniform2f(glState.program.uniforms.resolution, canvas.width / devicePixelRatio, canvas.height / devicePixelRatio);
	}
...
${new Array(glState.MAX_TEXTURE_UNITS).fill(0).map((v, i) => {
	...
	function invalidateImageCache(img) {
		glState.imageTextureMap.delete(img);
	}
	...
	const components = {
		...
		texturedTriangle, texturedQuad, texturedEllipse, texturedPolygon, invalidateImageCache,
